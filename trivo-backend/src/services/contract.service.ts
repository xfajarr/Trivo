...
export async function updatePrice(pair: string, price: number) {
  ...
  const tx = await wc.writeContract({
    ...
    abi: [{
      type: 'function' as const, name: 'updatePrice',
...
export async function getPrice(pair: string): Promise<number> {
  ...
  const result = await publicClient.readContract({
    address: config.SIMPLE_ORACLE as `0x${string}`,
    abi: [{
      type: 'function' as const, name: 'getPrice',
...
export async function registerAgentOnChain(agentId: number, agentAddress: string, agentOwner: string) {
  ...
  const tx = await wc.writeContract({
    ...
    abi: [{
      type: 'function' as const, name: 'registerAgent',
      inputs: [
...
export async function reportPositionOnChain(
  agentId: number, venue: string, market: string, side: string,
  size: number, entryPrice: number, leverage: number
  ...
  const tx = await wc.writeContract({
    address: config.COPY_TRADING as `0x${string}`,
    abi: [{
      type: 'function' as const, name: 'reportPosition',
      inputs: [
        { name: 'agentId', type: 'uint256' }, { name: 'venue', type: 'string' },
        { name: 'market', type: 'string' }, { name: 'side', type: 'string' },
        { name: 'size', type: 'uint256' }, { name: 'entryPrice', type: 'uint256' },
        { name: 'leverage', type: 'uint256' }, { name: 'refId', type: 'bytes32' },
      ],
...
export async function attachFollowerOnChain(follower: string, targetAgentId: number, allocationBps: number) {
  ...
  const tx = await wc.writeContract({
    address: config.COPY_TRADING as `0x${string}`,
    abi: [{
      type: 'function' as const, name: 'attachFollower',
      inputs: [
