...
const STYLES = [
  { id: "conservative", label: "Conservative", desc: "Low leverage, capital preservation", levMul: 0.5 },
  { id: "balanced", label: "Balanced", desc: "Moderate sizing, mix of yield and directional", levMul: 1 },
  { id: "aggressive", label: "Aggressive", desc: "High leverage, momentum, max upside", levMul: 1.8 },
];
...
export const Route = createFileRoute("/launch")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Configure a programmable AI trading agent with on-chain identity." },
    ],
  }),
    ...
    <div className="relative">
      ...
      <div className="relative mx-auto max-w-3xl px-4 py-6 sm:py-8">
        <div className="mb-6">
          ...
          <p className="text-sm text-muted-foreground mt-1">Configure your AI trading agent with on-chain identity</p>
        </div>
        ...
        <ol className="mb-6 flex items-center gap-2 overflow-x-auto sm:mb-8">
          {STEPS.map((s, i) => (
            <li key={s} className="flex flex-1 min-w-fit items-center gap-2">
              <div className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full border ticker text-xs ${
                i < step ? "border-neon bg-neon/10 text-neon" :
                i === step ? "border-neon bg-neon text-primary-foreground" :
        ...
        <div className="rounded-xl border border-border bg-card p-5 sm:p-6">
          ...
          {step === 0 && (
            <div className="space-y-5">
              <div>
                <Label className="ticker text-xs uppercase tracking-widest text-muted-foreground mb-2 block">Agent Name</Label>
                <Input value={name} onChange={e => { setName(e.target.value); if (!handle) setHandle(e.target.value.toLowerCase().replace(/[^a-z0-9_-]/g, "")); }} placeholder="e.g., MomentumMaster" maxLength={32} />
              </div>
              <div>
                <Label className="ticker text-xs uppercase tracking-widest text-muted-foreground mb-2 block">Handle</Label>
                <Input value={handle} onChange={e => setHandle(e.target.value.toLowerCase().replace(/[^a-z0-9_-]/g, ""))} placeholder="momentummaster" maxLength={32} />
                <p className="text-[10px] text-muted-foreground mt-1">Unique identifier. Used in URLs and mentions.</p>
              </div>
              <div>
                <Label className="ticker text-xs uppercase tracking-widest text-muted-foreground mb-2 block">AI Model</Label>
                <div className="grid gap-2 sm:grid-cols-3">
                  {MODELS.map(m => (
                    <button key={m.id} onClick={() => setModel(m.id)} className={`rounded-lg border p-3 text-left transition-colors ${model === m.id ? "border-neon/60 bg-neon/5" : "border-border hover:border-neon/30"}`}>
          ...
          {step === 1 && (
            <div className="space-y-5">
              <div>
                <Label className="ticker text-xs uppercase tracking-widest text-muted-foreground mb-2 block">Trading Venues</Label>
                <p className="text-xs text-muted-foreground mb-3">Select where your agent can trade</p>
                <div className="grid gap-2 sm:grid-cols-3">
                  {VENUES.map(v => (
                    <button key={v.id} onClick={() => toggleVenue(v.id)} className={`rounded-lg border p-3 text-left transition-colors ${venues.includes(v.id) ? "border-neon/60 bg-neon/5" : "border-border hover:border-neon/30"}`}>
              ...
              <div>
                <Label className="ticker text-xs uppercase tracking-widest text-muted-foreground mb-2 block">Strategy</Label>
                <Textarea value={strategy} onChange={e => setStrategy(e.target.value)} placeholder="Describe your strategy in plain English, e.g.: Trade BTC momentum with 2x leverage. Buy when RSI < 30 and MACD bullish. Take profit at 2%, stop loss at 1%." rows={4} className="text-sm" />
              </div>
              <div>
                <Label className="ticker text-xs uppercase tracking-widest text-muted-foreground mb-2 block">Signals</Label>
                <div className="flex flex-wrap gap-2">
                  {SIGNALS.map(s => (
                    <button key={s.id} onClick={() => toggleSignal(s.id)} className={`h-7 px-3 rounded border ticker text-[10px] tracking-wider transition-colors ${signals.includes(s.id) ? "border-neon bg-neon/10 text-neon" : "border-border bg-transparent text-muted-foreground hover:text-foreground"}`}>
                      {s.label}
                    </button>
                  ))}
                </div>
          ...
          {step === 2 && (
            <div className="space-y-5">
              <div>
                <Label className="ticker text-xs uppercase tracking-widest text-muted-foreground mb-2 block">Risk Style</Label>
                <div className="grid gap-2 sm:grid-cols-3">
                  {STYLES.map(s => (
                    <button key={s.id} onClick={() => setStyle(s.id)} className={`rounded-lg border p-3 text-left transition-colors ${style === s.id ? "border-neon/60 bg-neon/5" : "border-border hover:border-neon/30"}`}>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <Slider value={[budget]} onValueChange={v => setBudget(v[0])} min={10} max={1000} step={10} />
                </div>
                <div>
                  <Label className="ticker text-xs uppercase tracking-widest text-muted-foreground mb-2 block">
                ...
                <div>
                  ...
                  <Slider value={takeProfit} onValueChange={setTakeProfit} min={1} max={50} step={1} />
                </div>
              </div>
              ...
              <div className="flex items-center justify-between rounded-lg border border-border p-4">
                <div>
                  ...
                  <div className="text-[11px] text-muted-foreground">Other agents can mirror your positions. You earn fees.</div>
                </div>
          ...
          {step === 3 && (
            <div className="space-y-5">
              <div className="rounded-lg border border-neon/30 bg-neon/5 p-4">
                <div className="font-display text-lg font-semibold text-neon">{name || "Untitled agent"}</div>
              ...
              <div className="rounded-lg border border-border bg-surface-2 p-4 text-xs text-muted-foreground">
                Your agent will be deployed with ERC-8004 on-chain identity on Arc Testnet. 
                All trades use real market data from CoinGecko. Mock execution — no real USDC at risk.
          ...
          <div className="mt-8 flex items-center justify-between">
            ...
            {step < STEPS.length - 1 && (
              <Button onClick={() => setStep(s => s + 1)} disabled={!canNext} className="bg-neon text-primary-foreground hover:bg-neon/90">
                Next
              </Button>
            )}
          </div>
