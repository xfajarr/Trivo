  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8">
      ...
      <div className="mb-6 grid gap-px overflow-hidden rounded-lg border border-border bg-border sm:grid-cols-4">
        ...
        <Tile
          label="Realized"
          value={`${totalRealized >= 0 ? "+" : ""}$${Math.abs(totalRealized).toLocaleString()}`}
          tone={totalRealized >= 0 ? "neon" : "loss"}
        />
        <Tile
          label="Unrealized"
          value={`${totalUnrealized >= 0 ? "+" : ""}$${Math.abs(totalUnrealized).toLocaleString()}`}
          tone={totalUnrealized >= 0 ? "neon" : "loss"}
        />
            ...
            >
              <div className="flex items-start gap-3">
                <Link
                  to="/agent/$id"
                  params={{ id: a.id }}
                  className="flex h-12 w-12 shrink-0 items-center justify-center rounded-md border border-border bg-surface-2 font-display text-xl font-bold"
                >
                  {a.name?.[0] || "?"}
                </Link>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <Link
                      to="/agent/$id"
                      params={{ id: a.id }}
                      className="font-display text-lg font-semibold hover:text-neon truncate"
                    >
                      {a.name}
                    </Link>
                    <Badge
                      variant="outline"
                      className={`ticker text-[10px] shrink-0 ${isActive ? "border-neon/40 text-neon" : "border-amber-400/40 text-amber-400"}`}
                    >
                      <span
                        className={`h-1.5 w-1.5 rounded-full mr-1 inline-block ${isActive ? "bg-neon animate-pulse" : "bg-amber-400"}`}
                      />
              ...
              <div className="mt-4 grid grid-cols-2 gap-2 rounded-md border border-border bg-surface-2/50 p-3 text-xs">
                <Cell
                  ...
                  tone={realized >= 0 ? "neon" : "loss"}
                />
                <Cell
                  label="Unrealized"
                  value={`${unrealized >= 0 ? "+" : ""}$${Math.abs(unrealized).toLocaleString()}${openPositions > 0 ? ` · ${openPositions}` : ""}`}
                  tone={unrealized >= 0 ? "neon" : "loss"}
                />
              </div>
...
function Tile({ label, value, tone }: { label: string; value: string; tone?: "neon" | "loss" }) {
  return (
    <div className="bg-card p-4">
      ...
      <div
        className={`ticker mt-1 text-xl font-semibold ${tone === "neon" ? "text-neon" : tone === "loss" ? "text-loss" : ""}`}
      >
        {value}
      </div>
    </div>
