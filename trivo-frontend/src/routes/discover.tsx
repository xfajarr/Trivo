    ...
    <div className="mx-auto max-w-7xl px-4 py-6">
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{agents.length} agents · {filtered.length} showing</p>
        </div>
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        {["all","testnet","mainnet"].map(m => (
          <button key={m} onClick={() => setMode(m as ModeFilter)} className={`h-8 px-3 rounded border ticker text-[11px] tracking-wider transition-colors ${mode === m ? "border-neon bg-neon/10 text-neon" : "border-border bg-transparent text-muted-foreground hover:text-foreground"} ${m === "mainnet" ? "opacity-50 cursor-not-allowed" : ""}`}>
            {m.toUpperCase()}{m === "mainnet" && <span className="text-[9px] text-amber-400 ml-0.5"> SOON</span>}
          </button>
        ))}
      </div>
      <div className="mb-4 flex flex-wrap gap-2">
        {(["pnl","aum","winRate","copiers"] as SortKey[]).map(key => (
          <button key={key} onClick={() => setSortBy(key)} className={`h-7 px-3 rounded border ticker text-[10px] tracking-wider ${sortBy === key ? "border-violet bg-violet/10 text-violet" : "border-border bg-transparent text-muted-foreground hover:text-foreground"}`}>
              ...
              {sorted.map(a => {
                ...
                return (
                  <tr key={a.id} className="border-b border-border last:border-0 hover:bg-surface-2/30 transition-colors">
                    ...
                    <td className={`px-4 py-3 text-right ticker font-semibold ${realized >= 0 ? "text-neon" : "text-loss"}`}>{realized >= 0 ? "+" : ""}${Math.abs(realized).toLocaleString()}</td>
                    <td className={`px-4 py-3 text-right ticker ${unrealized >= 0 ? "text-neon" : "text-loss"}`}>
                      {unrealized >= 0 ? "+" : ""}${Math.abs(unrealized).toLocaleString()}
                      {openPositions > 0 && <span className="text-[9px] text-muted-foreground"> · {openPositions}</span>}
                    </td>
                    ...
                    <td className="px-4 py-3 text-right ticker text-xs text-muted-foreground">{Number(a.copiers || 0)}</td>
      ...
      {!isLoading && sorted.length > 0 && <p className="mt-3 text-xs text-muted-foreground text-right">PnL refreshes every 10s · Arc Testnet</p>}
