...
function DiscoverPage() {
  ...
  // Fetch unrealized PnL for all agents (same as My Agents)
  const { data: pnlData } = useQuery({
    queryKey: ["pnl-summary-discover"],
    queryFn: async () => {
  ...
  const sorted = useMemo(() => {
    return [...filtered].sort((a, b) => {
      switch (sortBy) {
        case "pnl": {
          const pnlA = Number(a.totalPnl || 0) + (pnlData?.[a.id]?.unrealizedPnl || 0);
          const pnlB = Number(b.totalPnl || 0) + (pnlData?.[b.id]?.unrealizedPnl || 0);
          return pnlB - pnlA;
        }
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6">
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        <button onClick={() => setMode("all")} className={`inline-flex items-center gap-1.5 h-8 px-3 rounded border ticker text-[11px] tracking-wider transition-colors ${mode === "all" ? "border-neon bg-neon/10 text-neon" : "border-border bg-transparent text-muted-foreground hover:text-foreground"}`}>ALL</button>
        <button onClick={() => setMode("testnet")} className={`inline-flex items-center gap-1.5 h-8 px-3 rounded border ticker text-[11px] tracking-wider transition-colors ${mode === "testnet" ? "border-neon bg-neon/10 text-neon" : "border-border bg-transparent text-muted-foreground hover:text-foreground"}`}>TESTNET</button>
        <button onClick={() => setMode("mainnet")} className={`inline-flex items-center gap-1.5 h-8 px-3 rounded border ticker text-[11px] tracking-wider transition-colors ${mode === "mainnet" ? "border-neon bg-neon/10 text-neon" : "border-border bg-transparent text-muted-foreground hover:text-foreground opacity-50 cursor-not-allowed"}`} title="Coming soon">MAINNET <span className="text-[9px] text-amber-400 ml-0.5">SOON</span></button>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-lg border border-border bg-card p-12 text-center">
          ...
          <Button onClick={() => setShowCreateModal(true)} className="bg-neon text-primary-foreground hover:bg-neon/90">Create Agent</Button>
        </div>
      ) : (
              ...
              {sorted.map((a) => {
                ...
                return (
                  <tr key={a.id} className="border-b border-border last:border-0 hover:bg-surface-2/30 transition-colors">
                    ...
                    <td className="px-4 py-3"><div className="flex flex-wrap gap-1">{skills.slice(0,2).map(s => <Badge key={s} variant="outline" className="ticker text-[10px] border-border bg-surface-2/60">{s}</Badge>)}</div></td>
                    <td className={`px-4 py-3 text-right ticker font-semibold ${realizedPnl >= 0 ? "text-neon" : "text-loss"}`}>{realizedPnl >= 0 ? "+" : ""}${Math.abs(realizedPnl).toLocaleString()}</td>
                    <td className={`px-4 py-3 text-right ticker ${unrealizedPnl >= 0 ? "text-neon" : "text-loss"}`}>
                      {unrealizedPnl >= 0 ? "+" : ""}${Math.abs(unrealizedPnl).toLocaleString()}
                      {openPositions > 0 && <span className="text-[9px] text-muted-foreground ml-0.5">· {openPositions}</span>}
                    </td>
                    ...
                    <td className="px-4 py-3 text-right"><span className="ticker text-xs text-muted-foreground">{copiers}</span></td>
      ...
      {!isLoading && sorted.length > 0 && (
        <p className="mt-3 text-xs text-muted-foreground text-right">
          PnL auto-refreshes every 30s · <span className="text-violet">Arc Testnet</span> · Mock execution with real prices
        </p>
      )}
