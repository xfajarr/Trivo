...
export function AgentChat({ agentId, agentName }: { agentId: string; agentName: string }) {
  const [messages, setMessages] = useState<Message[]>([
    { role: "assistant", content: `Hi! I'm ${agentName}. You can chat with me to refine my trading strategy or ask about my performance.` }
  ]);
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {messages.map((m, i) => (
          <div key={i} className={`flex gap-2 ${m.role === "user" ? "justify-end" : ""}`}>
            ...
            <div className={`rounded-lg px-3 py-2 text-xs max-w-[80%] ${m.role === "user" ? "bg-neon/10 text-neon" : "bg-surface-2 text-foreground"}`}>{m.content}</div>
